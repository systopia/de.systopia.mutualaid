...
class CRM_Mutualaid_CustomData
{
    ...
    protected function log($level, $message)
    {
        if ($level >= CUSTOM_DATA_HELPER_LOG_LEVEL) {
            CRM_Core_Error::debug_log_message(
              "CustomDataHelper {$this->version} ({$this->ts_domain}): {$message}"
            );
        }
    }
    ...
    public function syncEntities($source_file)
    {
        ...
        if (empty($data)) {
            throw new Exception(
              "syncOptionGroup::syncOptionGroup: Invalid specs"
            );
        }
        ...
        foreach ($data['_entities'] as $entity_data) {
            ...
            if (empty($entity)) {
                ...
                // Couldn't identify:
                $this->log(
                  CUSTOM_DATA_HELPER_LOG_ERROR,
                  "Couldn't create/update {$data['entity']}: " . json_encode(
                    $entity_data
                  )
                );
            } else {
    ...
    public function readResource($source_file) {
        $data = json_decode(file_get_contents($source_file), true);
        if (empty($data)) {
            throw new Exception(
                "syncOptionGroup::syncOptionGroup: Invalid specs"
    ...
    public function identifyCustomGroup($source_file) {
        $data = $this->readResource($source_file);
        ...
        if (isset($data['extends_entity_column_value'])) {
            ...
            if ($data['extends'] == 'Activity') {
                ...
                foreach ($data['extends_entity_column_value'] as $activity_type) {
                    if (!is_numeric($activity_type)) {
                        $activity_type = CRM_Core_OptionGroup::getValue(
                            'activity_type',
                            $activity_type,
                            'name'
                        );
                ...
                foreach ($data['extends_entity_column_value'] as $relationship_id) {
                    if (!is_numeric($relationship_id)) {
                        try {
                            ...
                            $this->log(
                                CUSTOM_DATA_HELPER_LOG_ERROR,
                                "Couldn't find relationship type '{$relationship_id}': " . $ex->getMessage(
                                )
                            );
                        }
                    }
            ...
            if (is_array($data['extends_entity_column_value'])) {
                $data['extends_entity_column_value'] = CRM_Utils_Array::implodePadded(
    ...
    public function identifyCustomField(&$customFieldSpec, &$customGroup) {
        ...
        if (!empty($customFieldSpec['option_group_id']) && !is_numeric(
                $customFieldSpec['option_group_id']
            )) {
            // look up custom group id
            $optionGroup = $this->getEntityID(
                'OptionGroup',
                array('name' => $customFieldSpec['option_group_id'])
            );
            if ($optionGroup == 'FAILED' || $optionGroup == null) {
                $this->log(
                    CUSTOM_DATA_HELPER_LOG_ERROR,
                    "Couldn't identify CustomField, bad option_group: {$customFieldSpec['option_group_id']}"
                );
                return;
            }
            $customFieldSpec['option_group_id'] = $optionGroup['id'];
        }
        $customField = $this->identifyEntity(
            'CustomField',
            $customFieldSpec
        );
    ...
    public function syncOptionGroup($source_file)
    {
        ...
        if (empty($optionGroup)) {
            ...
            $this->log(
              CUSTOM_DATA_HELPER_LOG_ERROR,
              "Couldn't create/update OptionGroup: " . json_encode(
                $data
              )
            );
            return;
        ...
        foreach ($data['_values'] as $optionValueSpec) {
            ...
            $optionValue = $this->identifyEntity(
              'OptionValue',
              $optionValueSpec
            );
            ...
            if (empty($optionValue)) {
                ...
                $this->log(
                  CUSTOM_DATA_HELPER_LOG_ERROR,
                  "Couldn't create/update OptionValue: " . json_encode(
                    $optionValueSpec
                  )
                );
            } else {
    ...
    public function syncCustomGroup($source_file)
    {
        list(
            ...
            $customGroup,
            $force_update
            ) = $this->identifyCustomGroup($source_file);
        ...
        if (empty($customGroup)) {
            ...
            $this->log(
              CUSTOM_DATA_HELPER_LOG_ERROR,
              "Couldn't create/update CustomGroup: " . json_encode(
                $data
              )
            );
            return;
        ...
        foreach ($data['_fields'] as $customFieldSpec) {
            $customField = $this->identifyCustomField(
                $customFieldSpec,
                $customGroup
            );
            if (empty($customField)) {
                // create CustomField
                $customField = $this->createEntity(
                  'CustomField',
                  $customFieldSpec
                );
                ...
                $this->log(
                  CUSTOM_DATA_HELPER_LOG_ERROR,
                  "Couldn't create/update CustomField: " . json_encode(
                    $customFieldSpec
                  )
                );
            } else {
    ...
    public function getCustomGroup($source_file) {
    ...
    protected function getEntityID($entity_type, $selector)
    {
        ...
        switch ($lookup_result['count']) {
            ...
            default:
                ...
                $this->log(
                  CUSTOM_DATA_HELPER_LOG_ERROR,
                  "Bad {$entity_type} lookup selector: " . json_encode(
                    $selector
                  )
                );
                return 'FAILED';
    ...
    protected function identifyEntity($entity_type, $data)
    {
        ...
        foreach ($data['_lookup'] as $lookup_key) {
            $lookup_query[$lookup_key] = CRM_Utils_Array::value(
              $lookup_key,
              $data,
              ''
            );
        ...
        $this->log(
          CUSTOM_DATA_HELPER_LOG_DEBUG,
          "LOOKUP {$entity_type}: " . json_encode($lookup_query)
        );
    ...
    protected function createEntity($entity_type, $data)
    {
        ...
        // then run query
        CRM_Core_Error::debug_log_message(
          "CustomDataHelper ({$this->ts_domain}): CREATE {$entity_type}: " . json_encode(
            $data
          )
        );
    ...
    protected function updateEntity(
      ...
      $force = false
        ...
        if (isset($requested_data['_no_override']) && is_array(
            $requested_data['_no_override']
          )) {
        ...
        if ($force || !empty($update_query)) {
            ...
            $this->log(
              CUSTOM_DATA_HELPER_LOG_INFO,
              "UPDATE {$entity_type}: " . json_encode($update_query)
            );
    ...
    protected function translateStrings(&$data)
    {
        ...
        foreach ($data['_translate'] as $translate_key) {
            ...
            if (is_string($value)) {
                $data[$translate_key] = ts(
                  $value,
                  array('domain' => $this->ts_domain)
                );
            }
        }
    }
        ...
        foreach ($data as $key => $value) {
            if (preg_match(
              ...
              $match
                ...
                if (empty($customgroups) || in_array(
                    $match['group_name'],
                    $customgroups
                  )) {
     ...
     */
    public static function getCustomFieldKey(
      $custom_group_name,
      $custom_field_name
    ) {
     ...
     * "LEFT JOIN {$group_table_name} AS {$table_alias} ON
     * {$table_alias}.entity_id = {$join_entity_id}"
     */
    public static function createSQLJoin(
      $group_name,
      $table_alias,
      $join_entity_id
    ) {
    ...
    /**
     * Set a field value in CiviCRM's pre-hook structure right in the pre hook
     * data
     *
     * @param $params pre-hook data
     * @param $field_id custom field ID
     ...
     */
    public static function setPreHookCustomDataValue(
      &$params,
      $field_id,
      $value
    ) {
        if ($field_id) {
            if (isset($params['custom'])) {
                if (!empty($params['custom'][$field_id][-1])) {
                    ...
                    $params['custom'][$field_id][-1] = self::generatePreHookCustomDataRecord(
                      $field_id,
                      $value
                    );
                }
    ...
    protected static function generatePreHookCustomDataRecord($field_id, $value)
    {
        if ($field_id) {
            ...
            return [
              'value' => $value,
              'type' => CRM_Utils_Array::value(
                'data_type',
                $field_specs,
                'String'
              ),
              'custom_field_id' => $field_id,
              'custom_group_id' => CRM_Utils_Array::value(
                'custom_group_id',
                $field_specs,
                null
              ),
              'table_name' => CRM_Utils_Array::value(
                'table_name',
                $group_specs,
                null
              ),
              'column_name' => CRM_Utils_Array::value(
                'column_name',
                $field_specs,
                null
              ),
              'is_multiple' => CRM_Utils_Array::value(
                'is_multiple',
                $group_specs,
                0
              ),
