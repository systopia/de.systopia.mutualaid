 ...
 */
class CRM_Mutualaid_Upgrader_Base
{
    ...
    static public function instance()
    {
        if (!self::$instance) {
            // FIXME auto-generate
            self::$instance = new CRM_Mutualaid_Upgrader(
              'de.systopia.mutualaid',
              realpath(__DIR__ . '/../../../')
            );
        }
     ...
     *
     * @code
     * CRM_Mutualaid_Upgrader_Base::_queueAdapter($ctx, 'methodName', 'arg1',
     *   'arg2');
     * @endcode
     */
     ...
     *
     * @param string $relativePath the CustomData XML file path (relative to
     *   this extension's dir)
     *
     * @return bool
     */
    ...
    {
        $revision = CRM_Core_BAO_Extension::getSchemaVersion(
          $this->extensionName
        );
        if (!$revision) {
    ...
    {
        CRM_Core_BAO_Extension::setSchemaVersion(
          $this->extensionName,
          $revision
        );
